...
function ppefw_payeasy_assets_url()
{
    return plugin_dir_url(dirname(__FILE__)) . 'assets/';
}

class PPEFW_Payeasy_Gateway extends WC_Payment_Gateway
{
    public $instructions;
    public $testmode;
    public $api_key;
    ...
    public function __construct()
    {
        ...
        $this->id   = 'payeasy';
        $this->icon = apply_filters('woocommerce_payeasy_icon',  ppefw_payeasy_assets_url() . 'images/logo-pix.png');
        $this->has_fields = false;
        $this->method_title = __('Pay Easy Pix', 'pix-pay-easy-for-woocommerce');
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields = apply_filters('woo_payeasy_fields', array(
            ...
            'title' => array(
                ...
                'type' => 'text',
                'default' => __('Pay Easy Pix', 'pix-pay-easy-for-woocommerce'),
                'desc_tip' => true,
                'description' => __('Add a new title for the Pay Easy Pix that customers will see when they are in the checkout page.', 'pix-pay-easy-for-woocommerce')
            ),
            'description' => array(
                'title' => __('Pay Easy Pix Description', 'pix-pay-easy-for-woocommerce'),
                'type' => 'textarea',
                'default' => 'Você será redirecionado para realizar o pagamento do QR CODE',
                'desc_tip' => true,
                'description' => __('Add a new description for the Pay Easy Pix that customers will see when they are in the checkout page.', 'pix-pay-easy-for-woocommerce')
            ),
            'instructions' => array(
                'title' => __('Instructions', 'pix-pay-easy-for-woocommerce'),
                'type' => 'textarea',
                'default' => __('Abra o aplicativo do seu banco e escaneie o código QR para pagar ou copie o Pix, copie e cole abaixo e cole no seu aplicativo para finalizar a compra.', 'pix-pay-easy-for-woocommerce'),
                'desc_tip' => true,
    ...
    {
        if (!isset($_POST['payeasy_payment_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['payeasy_payment_nonce'])), 'payeasy_payment_process')) {
            wc_add_notice(__('Error: Nonce verification failed, please try again.', 'pix-pay-easy-for-woocommerce'), 'error');
            return;
        }
        $order = wc_get_order($order_id);
        $payeasy_cpf = isset($_POST['payeasy_cpf']) ? sanitize_text_field($_POST['payeasy_cpf']) : '';
        ...
        // call payeasy api
        $response = wp_remote_post($base_url . '/add/deposit-pix?api_key=' . $this->api_key, $args);
        ...
        if (!is_wp_error($response)) {
            ...
            if ($body['status'] === 'created') {
                ...
                wc_add_notice(__('The PIX was not generated. Contact with the payment provider.', 'pix-pay-easy-for-woocommerce'), 'error');
                return;
            }
        } else {
...
?>
        <script>
            jQuery(document).ready(function($) {
                const formatCPF = (cpf) => {
                    let cpfInput = document.getElementById("payeasy_cpf");
                ...
                if (billingCpfValue?.length > 0) {
                    $("#payeasy_cpf").val(billingCpfValue);
                }
                $("#payeasy_cpf").on("input", function() {
                    formatCPF()
                });
                $("#billing_cpf").on("input", function() {
                    var value = $("#billing_cpf").val();
                    $("#payeasy_cpf").val(value);
                });
            });
        </script>
<?php

        wp_nonce_field('payeasy_payment_process', 'payeasy_payment_nonce');
        ...
        <div class="form-row form-row-wide"><label>CPF / CNPJ<span class="required">*</span></label>
            <input id="payeasy_cpf" type="text" placeholder="' . esc_attr(__('Enter your CPF / CNPJ ', 'pix-pay-easy-for-woocommerce')) . '" name="payeasy_cpf">
            </div>
            <br>
            <div class="form-row form-row-wide">
            <input id="payeasy_terms" type="checkbox" name="payeasy_terms" checked>
    ...
    public function custom_thankyou_qrcode_content($str, $order_id)
    {
        ...
        if ($order && "wc-" . $order_status === $order_status_pending) {
            $payeasy_pix_qr_link = $order->get_meta('payeasy_pix_qr_link');
            $payeasy_pix_amount = $order->get_meta('payeasy_pix_amount');
            $payeasy_qr_code = $order->get_meta('payeasy_qr_code');

            if ($payeasy_pix_qr_link) {
                echo esc_attr(__('Please complete your payment', 'pix-pay-easy-for-woocommerce')) . '<div id="payeasy-send-payment">
                            <div class="payeasy-status">
                                <span><strong>' . esc_attr(__('PIX Payment', 'pix-pay-easy-for-woocommerce')) . '</strong></span>
                                <span id="blinkText" class="message">Aguardando pagamento</span>
                            </div>
                            <div class="payeasy-qr">
                                <div class="payeasy-qr">
                                    <img src="'. esc_html($payeasy_qr_code) .'" alt="QR Code" />
                                </div>
                                <span>' . esc_html($this->instructions) . '</span>
                            </div>
                            <div class="payeasy-address">
                                <span>' . esc_attr(__('PIX Code', 'pix-pay-easy-for-woocommerce')) . '</span>
                                <span><strong>' . esc_attr(__('Copy pix code', 'pix-pay-easy-for-woocommerce')) . '</strong></span>
                                <div class="payeasy-copy">
                                    <a href="#" data-copy="' . esc_html($payeasy_pix_qr_link) . '"></a>
    ...
    public function custom_thankyou_qrcode_enqueue_script()
    {
        if (is_order_received_page()) {
            ...
            wp_localize_script('custom-qrcode-script', 'payeasy_params', array(
                ...
                'logo_url' => ppefw_payeasy_assets_url() . 'images/logo96.png',
                'qr_code' => $payeasy_pix_qr_link
